...
function updateTime() {
  ...
  if (currentTime >= 390 && currentTime <= 460) { // Routine hours or test mode
    ...
    document.getElementById('time').textContent = `${formattedTime} - Routine terminée`;
...
document.addEventListener('keydown', (event) => {
  ...
  bravoMessage.textContent = 'Bravo, la routine est terminée dans les temps !';
  ...
  if (event.code === 'Space') {
    ...
    if (isLastStep && completedInGreen) {
      stepElement.textContent = 'Bravo, la routine est terminée dans les temps !';
    ...
    if (!isLastStep) {
      currentStepIndex++;
      document.getElementById('step').textContent = `Étape : ${steps[currentStepIndex].name}`;
      document.getElementById('step-timing').textContent = `Temps recommandé : ${steps[currentStepIndex].recommended}`;
    }
  }
});
